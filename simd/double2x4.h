...
#define CR_MACRO_DATA_TYPE double2x4
...
namespace cr::simd
{
	...
	struct double2x4
	{
		...
		CR_INLINE double2x4() = default;
		CR_INLINE double2x4(double s)
		    : f1(_mm256_set1_pd(s)),
		      f2(_mm256_set1_pd(s)) {
		}
		CR_INLINE double2x4(double const* ptr)
		    : f1(_mm256_loadu_pd(ptr)),
		      f2(_mm256_loadu_pd(ptr + 4)) {
		}
		CR_INLINE double2x4(double const* ptr, aligned_hint_t)
		    : f1(_mm256_load_pd(ptr)),
		      f2(_mm256_load_pd(ptr + 4)) {
		}
		CR_INLINE double2x4(__m256d f1_, __m256d f2_)
		    : f1(f1_),
		      f2(f2_) {
		}
		CR_INLINE double2x4(double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7)
		...
		DEFINE2(fma, fmadd)
#else
		CR_INLINE double2x4 fma(in_t(double2x4) a, in_t(double2x4) b) const {
			return (*this) * a + b;
		}
#endif
