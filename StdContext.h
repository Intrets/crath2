...
namespace fun
{
	...
	template<class F>
	inline constexpr static F atan_positive_quadrant_remez_fma_T5_0(in_t(F) x) {
		auto m = x > F(1.0f);
		auto a = forward_definitions::blend(F(0.0f), F(1.5707963267948966f), m);
		x = forward_definitions::blend(x, F(1.0f) / x, m);
		auto const a5 = F(-0.04949958068912102f);
	...
	inline static float atan_positive_quadrant_remez_fma_T5_0_float_simd(float x) {
#ifdef ARCH_x86
		return atan_positive_quadrant_remez_fma_T5_0<cr::simd::float1x4>(x).first();
#else
		return atan_positive_quadrant_remez_fma_T5_0<float>(x);
#endif
	}
}
...
namespace cr
{
	...
	struct StdContext : forward_definitions
	{
		...
		// domain: (-inf, inf)
		// maximum absolute error: 0.000021
		// maximum relative error:0.008771
		template<class F>
		...
		// maximum relative error:0.008771
		template<class F>
		CR_INLINE constexpr static F atan_positive(in_t(F) x) {
			if constexpr (std::same_as<F, float>) {
				if (std::is_constant_evaluated()) {
					return fun::atan_positive_quadrant_remez_fma_T5_0(x);
				}
				else {
					return fun::atan_positive_quadrant_remez_fma_T5_0_float_simd(x);
				}
			}
			else {
				return fun::atan_positive_quadrant_remez_fma_T5_0(x);
